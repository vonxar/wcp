...
public class Board2 {
	...
	ArrayList<RevList2> whiteList = new ArrayList<>();
	ArrayList<RevList2> blackList = new ArrayList<>();
		...
		public void findSetHeres (String stone) {
			...
			for (int i=1; i<=8; i++) {
				for (int j=1; j<=8; j++) {
					...
					if (board2[i][j].equals("ー ") == false) continue;

					//まず周囲八方向を見る。相手の石が1個もなければcontinue
					boolean around = false;
					for (int k=0; k<8; k++) {
						if (board2[i+D8[k].getY()][j+D8[k].getX()].equals(getEnemyStone(stone))) {
							around = true;
						}
					}
					...
					RevList2 revList = new RevList2(new Here(i, j));
					...
					for (int k=0; k<8; k++) {
						...
						while (true) {
							nowP.move(D8[k]);
							if (board2[nowP.getY()][nowP.getX()].equals(getEnemyStone(stone)) == true) {
								tempHeres.add(new Here(nowP.getY(), nowP.getX()));
							}
							else if (board2[nowP.getY()][nowP.getX()].equals(stone) == true) {
								if (tempHeres.size() > 0) {
									canCapture = true;
									break;
								}
...
//		}
			...
			for (int i=0; i<whiteList.size(); i++) {
				if (whiteList.get(i).getCenterY()==y && whiteList.get(i).getCenterX()==x) {
					...
					for (int j=0; j<list.size(); j++) {
						board2[list.get(j).getY()][list.get(j).getX()] = stone;
					}
					return;
				}
			}
	...
	public void result () {
		...
		for (int i=1; i<=8; i++) {
			for (int j=1; j<=8; j++) {
				if (board2[i][j].equals("⚪️")) whiteNum++;
				else if (board2[i][j].equals("⚫️")) blackNum++;
			}
		}
